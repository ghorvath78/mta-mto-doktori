...
import { Button, type ButtonProps } from "./dzbutton";
...
const useDropzone = <TUploadRes, TUploadError = string>(props: UseDropzoneProps<TUploadRes, TUploadError>): UseDropzoneReturn<TUploadRes, TUploadError> => {
    ...
    const _uploadFile = useCallback(
        async function uploadFile(file: File, id: string, tries = 0) {
...
const DropZoneArea = forwardRef<HTMLDivElement, DropZoneAreaProps>(({ className, children, ...props }, forwardedRef) => {
    ...
    return (
        // A11y behavior is handled through Trigger. All of these are only relevant to drag and drop.
        <div
            ref={(instance) => {
